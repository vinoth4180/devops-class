...
class Student:
    def __init__(self, root):
        ...
        def add_data():
            if self.roll_no.get() == "" or self.name.get() == "" or self.father_name.get() == "" or self.contact.get() == "" or self.address.get() == "" or len(self.contact.get()) != 10:
                ...
                con = sqlite3.connect(r'C:\Users\hp\training.db')
                cur = con.cursor()
                sql=f"insert into students1 values('{self.roll_no.get()}','{self.name.get()}','{self.father_name.get()}','{self.gen.get()}','{self.category.get()}','{self.branch.get()}','{ self.year.get()}','{self.contact.get()}','{self.address.get()}')"
                cur.execute(sql)
                con.commit()
                con.close()
                fetch_data()
