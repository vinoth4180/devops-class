...
def Ref():
    ...
    cur = conn.cursor()
    sql = f"insert into rest values('{rand.get()}','{Fries.get()}','{Largefries.get()}','{Burger.get()}','{Filet.get()}','{Cheese_burger.get()}','{Drinks.get()}','{cost.get()}','{Service_Charge.get()}','{Tax.get()}','{Subtotal.get()}','{Total.get()}')"
    cur.execute(sql)
    conn.commit()
    cur.close()
...
status = Label(f2,font=('aria', 15, 'bold'),width = 16, text="-By Amar Kumar",bd=2)
...
def price():
    ...
    lblinfo.grid(row=0, column=0)
    lblinfo = Label(roo, font=('aria', 15,'bold'), text="___", fg="white")
